...
# -*- coding: utf-8 -*-
"""
신규 스탬프 팩 엑셀 파일에서 개별 에셋 썸네일 이미지를 추출하는 스크립트
"""
...
def extract_thumbnails_from_excel():
    """엑셀 파일에서 개별 에셋 썸네일 이미지 추출"""
                ...
                for row_idx in range(header_row + 1, min(header_row + 21, next_section_row)):
                    try:
                        ...
                        if output_path.exists():
                            skipped_count += 1
                            asset_count += 1
                            print(f"  ⏭️  [{row_idx}] {filename} (이미 존재)")
                            continue
                        
                        # 해당 행의 IMG 컬럼(Col 3 = C열)에 이미지가 있는지 확인
                        ...
                        if cell_key in image_cell_map:
                            ...
                            # 같은 행의 다른 열에서 이미지 찾기 (IMG 컬럼 근처)
                            for (r, c), (rel_id, img_name) in image_cell_map.items():
                                if r == row_idx and abs(c - img_col) <= 1:  # 같은 행, 인접한 열
                                    if rel_id in rel_id_to_image:
                                        image_path = rel_id_to_image[rel_id]
                                        found_image = True
                                        break
                        ...
                        if found_image:
                            ...
                            skipped_count += 1
                            asset_count += 1
                            print(f"  ✗ [{row_idx}] {filename} (이미지 없음)")
...
def main():
    """메인 함수"""
    print("="*60)
    print("🖼️  신규 스탬프 팩 개별 에셋 썸네일 추출 스크립트")
    print("="*60)
    
    ensure_output_dir()
    
    success = extract_thumbnails_from_excel()
    
    if success:
        print("\n✅ 이미지 추출 완료!")
        print(f"📁 저장 위치: {OUTPUT_DIR}")
    else:
        print("\n⚠️  일부 이미지 추출 실패 또는 이미지가 없습니다")
        print("엑셀 파일의 이미지가 XML에 제대로 저장되어 있는지 확인해주세요.")
