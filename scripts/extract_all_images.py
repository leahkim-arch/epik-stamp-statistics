...
# -*- coding: utf-8 -*-
"""
신규 스탬프 팩 엑셀 파일에서 모든 이미지를 추출하는 스크립트 (openpyxl 직접 접근)
"""
...
def extract_all_images():
    ...
    try:
        # 이미지 추출을 위해 data_only=False로 로드
        ...
        # 이미지 매핑 생성 (worksheet._images 사용)
        image_map = {}  # (row, col) -> image object
                ...
                try:
                    if hasattr(img, 'anchor'):
                        ...
                        if hasattr(anchor, '_from'):
                            ...
                            if (row, col) not in image_map:
                                image_map[(row, col)] = []
                            image_map[(row, col)].append(img)
                            
                            # 디버깅: 개별 에셋 섹션의 이미지 출력
                            asset_rows = []
                            for s in sections:
                                asset_rows.extend(range(s['header_row'] + 1, s['header_row'] + 21))
                            
                            if row in asset_rows and col == 3:
                                print(f"  발견: Row {row}, Col {col}")
                except Exception as e:
                    print(f"  ⚠️  이미지 {idx+1} 매핑 오류: {e}")
                    continue
        
        print(f"✓ 매핑된 이미지 위치: {len(image_map)}개")
        ...
        for section_idx, section in enumerate(sections):
            ...
            for row_idx in range(header_row + 1, min(header_row + 21, next_section_row)):
                try:
                    ...
                    # 파일명 생성 (Categories 우선, 없으면 Title 사용)
                    ...
                    # 해당 행의 IMG 컬럼에 이미지가 있는지 확인
                    cell_key = (row_idx, img_col)
                    found_image = None
                    
                    # 정확한 위치 확인
                    if cell_key in image_map:
                        found_image = image_map[cell_key][0]
                    else:
                        # 같은 행의 다른 열에서 이미지 찾기
                        for (r, c), imgs in image_map.items():
                            if r == row_idx:
                                found_image = imgs[0]
                                break
...
def main():
    """메인 함수"""
    print("="*60)
    print("🖼️  신규 스탬프 팩 모든 이미지 추출 스크립트")
    print("="*60)
    ...
    if success:
        ...
        print("\n⚠️  일부 이미지 추출 실패")
        print("엑셀 파일을 다시 저장하면 더 많은 이미지를 추출할 수 있습니다.")
